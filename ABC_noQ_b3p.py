...
file_str = 'b3p_' + geometry + "_" + str(dimP) + ".txt"
...
dddp = inner(grad(ddp), n) # d^2p/dn^2 = grad(dp/dn) * n = grad(grad(p) * n) * n
...
for ii in tqdm(range(freqvec.size)):
    ...
    omega0 = 2*np.pi*freq
    k0 = omega0/c0
    d_0 = 18*fx1**2 - 3*k0**2 + 18j*k0*fx1 -1j*k0**3
    d_1 = 9*fx1 + 3j*k0
    d_2 = 6*fx1**3 - 9*k0**2*fx1 + 18j*k0*fx1**2 - 1j*k0**3
    coeff_1 = 1/d_0
    coeff_2 = d_1/d_0
    coeff_3 = d_2/d_0
    c = coeff_1*c_1 + coeff_2*c_2 + coeff_3*c_3
